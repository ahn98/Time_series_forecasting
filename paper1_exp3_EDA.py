################
'''
- 온실에서 수집된 15가지 데이터(온실 내부 센서 정보 + 온실 외부 센서 정보 + 액츄에이터 정보)를 활용하여 온실 내부의 "온도", "습도", "CO2 농도"의 각각의 변화량(30분 이후)을 예측한다.
- input length = 30days / output length = after 30min
- 전처리 후 dataframe time step: 30 min
- 원본 데이터 수: 47,809
- 전처리 후 데이터 수: 47,809
'''
################
